...
超时 对于一个连接外部资源，或者其它一些需要花费执行时间的操作的程序而言是很重要的。得益于**通道** 和 `select`，在 Go中实现超时操作是简洁而优雅的。
...
`res := <- c1` 等待结果，`<-Time.After` 等待超时时间 1 秒后发送的值。由于 select 默认处理第一个已准备好的接收操作，如果这个操作超过了允许的 1 秒的话，将会执行超时 case。
